...
with open(r"input_data/11.json", "r") as f:
    data = json.load(f)
...
def __integr_func(
        ...
        dp_dl = calc_dp_dl(rho_w=rho_w, mu_w=mu_w, angle=angle, q_ms=q_ms, d_tub=d_tub, roughness=roughness)
        dt_dl = temp_grad / 100
...
def calc_pipe(
        ...
        params = (temp_grad, gamma_wat, angle, q_ms, d_tub, roughness)
        y0 = (p_wh, t_wh)
        H = np.linspace(h0, md_vdp, 1000)
        t_span = (h0, md_vdp)
        sol = solve_ivp(fun=__integr_func, t_span=t_span, y0=y0, args=params, t_eval=H)
        p, t = sol.y
...
def calc_p_wf(
        ...
        results = calc_pipe(p_wh=p_wh, t_wh=t_wh, h0=0, md_vdp=md_vdp, temp_grad=temp_grad, gamma_wat=gamma_wat, angle=angle, q_ms=q_ms, d_tub=d_tub, roughness=roughness)
...
if __name__ == "__main__":
    ...
    results = calc_pipe(p_wh, t_wh+273, h0=0, md_vdp=md_vdp, temp_grad=temp_grad, gamma_wat=gamma_water, angle=angle, q_ms=q_m3_sec, d_tub=d_tub, roughness=roughness)
    ...
    VLP = lambda x: calc_pipe(p_wh, t_wh+273, h0=0, md_vdp=md_vdp, temp_grad=temp_grad, gamma_wat=gamma_water, angle=angle, q_ms=x, d_tub=d_tub, roughness=roughness)
    Q = np.arange(0, 400, 10)
    P = np.array([VLP(q)[0][-1] for q in Q / 86400]) / 101325

    plt.plot(Q, P)
    plt.xlabel('Q, Дебит нагнетательной скважины, м3/сут')
    plt.ylabel('P, Забойное давление')
    plt.title('VLP нагнетательной скважины')
    plt.show()

    np.array([1, 2, 3])
    output =  {"q_liq": Q.tolist(), "p_wf": P.tolist()}
    with open("output.json", "w", encoding="utf-8") as f:
        f.write(json.dumps(output, indent=2, ensure_ascii=True))